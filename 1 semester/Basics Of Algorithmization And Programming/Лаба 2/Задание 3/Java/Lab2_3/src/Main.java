...
public class Main {
    ...
    public static boolean checkArea(int num, int MIN, int MAX) {
        boolean isCorrect;
        if (num < MIN || num > MAX) {
    ...
    public static int readUserMatrix(int num, int row, int col, int MIN, int MAX) {
        ...
        do {
            ...
            if (isCorrect)
                isCorrect = checkArea(num, MIN, MAX);
        } while (!isCorrect);
        return num;
    }
    public static String readPathFile() {
        ...
        pathToFile = "";
        do {
            System.out.println("Введите путь к файлу с расширением .txt с матрицей, у которой числа должны соответствовать оценкам[" + MIN_MARK + ":" + MAX_MARK + "]: ");
    ...
    public static int[][] readConsoleMatrix() {
        ...
        for (int row = 0; row < LAST_STUDENT; row++)
            for (int col = 0; col < LAST_COLUMN; col++)
                do {
                    System.out.print("Введите оценку " + row + 1 + " человека за " + (col + 1) + " день:");
                    isCorrect = true;
                    try {
    ...
    public static void printConsoleResult(int[] goodMen) {
        int row;
        System.out.println("Номера отличников: ");

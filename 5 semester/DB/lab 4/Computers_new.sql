...
        ON UPDATE CASCADE ON DELETE RESTRICT -- RESTRICT: Нельзя удалить должность, если есть сотрудники
...
        ON UPDATE CASCADE ON DELETE RESTRICT, -- RESTRICT: Нельзя удалить клиента, если у него есть устройства
    CONSTRAINT fk_device_type FOREIGN KEY (device_type_id) REFERENCES device_type_lookup(type_id)
        ON UPDATE CASCADE ON DELETE RESTRICT -- RESTRICT: Нельзя удалить тип устройства, если к нему привязаны устройства
...
    engineer_id INT COMMENT 'Внешний ключ к employee (Инженер, назначенный на ремонт)', 
...
        ON UPDATE CASCADE ON DELETE RESTRICT, -- RESTRICT: Нельзя удалить устройство, если к нему привязан заказ
    CONSTRAINT fk_order_manager FOREIGN KEY (employee_id) REFERENCES employee(employee_id)
        ON UPDATE CASCADE ON DELETE RESTRICT, -- RESTRICT: Нельзя удалить менеджера, если он есть в активных заказах (ОБЯЗАТЕЛЬНОЕ ПОЛЕ)
    CONSTRAINT fk_order_engineer FOREIGN KEY (engineer_id) REFERENCES employee(employee_id)
        ON UPDATE CASCADE ON DELETE RESTRICT -- RESTRICT: Нельзя удалить инженера, если он есть в активных заказах. Можно изменить на ON DELETE SET NULL, так как поле необязательное, но RESTRICT безопаснее для учета.
...
    quantity TINYINT NOT NULL DEFAULT 1 COMMENT 'Количество выполненных услуг',
    actual_service_price DECIMAL(10, 2) NOT NULL COMMENT 'Фактическая цена, по которой оказана услуга',
    
    PRIMARY KEY (repair_order_id, service_id),
    
    CONSTRAINT fk_os_order FOREIGN KEY (repair_order_id) REFERENCES repair_order(order_id)
        ON UPDATE CASCADE ON DELETE CASCADE, -- CASCADE: Если заказ удален, удалить и его детали
    CONSTRAINT fk_os_service FOREIGN KEY (service_id) REFERENCES service(service_id)
        ON UPDATE CASCADE ON DELETE RESTRICT -- RESTRICT: Нельзя удалить услугу, если она где-то используется
...
    quantity TINYINT NOT NULL DEFAULT 1 COMMENT 'Количество использованных запчастей',
...
        ON UPDATE CASCADE ON DELETE RESTRICT -- RESTRICT: Нельзя удалить запчасть, если она где-то используется
) COMMENT 'Детализация использованных запчастей в заказе';


-- =================================================================
-- 5. ТРИГГЕРЫ (АВТОМАТИЧЕСКИЙ РАСЧЕТ total_amount) - Без изменений
-- =================================================================
DELIMITER //

-- Функция для пересчета общей суммы
CREATE FUNCTION calculate_total_amount(order_id_val INT)
RETURNS DECIMAL(10, 2)
READS SQL DATA
BEGIN
    DECLARE services_sum DECIMAL(10, 2);
    DECLARE parts_sum DECIMAL(10, 2);
    
    SELECT COALESCE(SUM(quantity * actual_service_price), 0)
    INTO services_sum
    FROM repair_order_service
    WHERE repair_order_id = order_id_val;
    
    SELECT COALESCE(SUM(quantity * actual_selling_price), 0)
    INTO parts_sum
    FROM repair_order_part
    WHERE repair_order_id = order_id_val;
    
    RETURN services_sum + parts_sum;
END //


-- Триггеры для repair_order_service (Услуги)
CREATE TRIGGER trg_os_insert AFTER INSERT ON repair_order_service
FOR EACH ROW
BEGIN
    UPDATE repair_order SET total_amount = calculate_total_amount(NEW.repair_order_id)
    WHERE order_id = NEW.repair_order_id;
END //

CREATE TRIGGER trg_os_update AFTER UPDATE ON repair_order_service
FOR EACH ROW
BEGIN
    UPDATE repair_order SET total_amount = calculate_total_amount(NEW.repair_order_id)
    WHERE order_id = NEW.repair_order_id;
...
CREATE TRIGGER trg_os_delete AFTER DELETE ON repair_order_service
FOR EACH ROW
BEGIN
    UPDATE repair_order SET total_amount = calculate_total_amount(OLD.repair_order_id)
    WHERE order_id = OLD.repair_order_id;
END //


-- Триггеры для repair_order_part (Запчасти)
CREATE TRIGGER trg_op_insert AFTER INSERT ON repair_order_part
FOR EACH ROW
BEGIN
    UPDATE repair_order SET total_amount = calculate_total_amount(NEW.repair_order_id)
    WHERE order_id = NEW.repair_order_id;
...
CREATE TRIGGER trg_op_update AFTER UPDATE ON repair_order_part
FOR EACH ROW
BEGIN
    UPDATE repair_order SET total_amount = calculate_total_amount(NEW.repair_order_id)
    WHERE order_id = NEW.repair_order_id;
...
CREATE TRIGGER trg_op_delete AFTER DELETE ON repair_order_part
FOR EACH ROW
BEGIN
 UPDATE repair_order SET total_amount = calculate_total_amount(OLD.repair_order_id)
 WHERE order_id = OLD.repair_order_id;
...
SELECT '✅ Физическая модель полностью оптимизирована и готова к работе.' AS Status;
...
begin
    ...
    TaskLabel.Caption :=
      'Данная программа вычисляет произведение элементов массива' + #13#10 +
      'c N элементов:';
...
Procedure ProductElements(StringGrid: TStringGrid);
Var
    I, Len: Integer;
    Product: Double;
    NumArr: TArr;
    NumsStringGrid: TEStringGrid;
Begin
    NumsStringGrid := TEStringGrid(StringGrid);
    // NumsStringGrid.InplaceEditor.Text :=
    Product := 1;
    Len := StrtoInt(MainTaskForm.EnterNEdit.Text);
    ...
    End;
    MainTaskForm.OutEdit.Text := FloatToStr(Product);
End;
...
Begin
    ...
    Begin
        if ColNum = 1 then
        Begin
            Grid.Width := (Grid.DefaultColWidth);
            Grid.Height := (Grid.DefaultRowHeight * 3) + 3;
        End
        Else
        Begin
            Grid.Width := (Grid.DefaultColWidth + 3) * ColNum;
            Grid.Height := (Grid.DefaultRowHeight * 2) + 3;
        End;
    End;
    Grid.Enabled := True;

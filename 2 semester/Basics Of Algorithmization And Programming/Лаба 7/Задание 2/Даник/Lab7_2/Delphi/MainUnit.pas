...
Begin
    ...
    Begin
        Grid.Cells[0, I] := Chr(Ord('A') + I - 1);
        Grid.Cells[I, 0] := Chr(Ord('A') + I - 1);
    End;
End;
...
Var
    ...
    I: Integer;
Begin
    For I := 0 To Length(AdjacencyArr) - 1 Do
    Begin
        While AdjacencyArr[I].Next <> Nil Do
        Begin
            CurrentNode := AdjacencyArr[I].Next;
            AdjacencyArr[I].Next := CurrentNode.Next;
            Dispose(CurrentNode);
        End;
        Dispose(AdjacencyArr[I]);
    End;
    SetLength(AdjacencyArr, 0);
End;
...
Var
    MatrixGrid: TStringGridEx;
    Text: String;
Begin
    MatrixGrid := TStringGridEx(Sender);
...
Var
    ...
    Temp: PVertex;
Begin
    SetLength(AdjacencyArr, Length(Matrix));
    For I := 0 To Length(AdjacencyArr) - 1 Do
    Begin
        New(AdjacencyArr[I]);
        AdjacencyArr[I].Next := Nil;
        Temp := AdjacencyArr[I];
        For J := 0 To Length(Matrix) - 1 Do
        Begin
            For K := 1 To Matrix[I, J] Do
            Begin
                New(Temp.Next);
                Temp := Temp.Next;
                Temp.Value := Chr(Ord('A') + J);
                Temp.Next := Nil;
            End;
        End;
    End;
...
Begin
    ...
    If RowNum < 4 Then
    Begin
        Grid.Height := (Grid.DefaultRowHeight + 4 - Grid.GridLineWidth) * Grid.RowCount + 2;
...
Begin
    If ACol = 1 Then
    Begin
        Text := ResultGrid.Cells[ACol, ARow];
        ResultGrid.Canvas.Brush.Color := ClWindow;
        ResultGrid.Canvas.FillRect(Rect);
        DrawText(ResultGrid.Canvas.Handle, PChar(Text), Length(Text), Rect,

...
/**
 * Middleware валидации депозитного договора.
 * Нормализует тело запроса, проверяет обязательность и форматы, а также
 * сверяет срока/ставку с депозитными программами и дату с банковской датой.
 */
...
module.exports = async function validateContract(req, res, next) {
  ...
  if (data.amount === null) {
    ...
    errors.amount = 'Сумма депозита: неотрицательное число с не более чем 2 знаками после запятой';
  ...
  try {
    ...
    const termRes = await pool.query(
      `SELECT pt.annual_rate, pt.term_months, p.deposit_type, p.interest_payment, p.name AS program_name
         FROM deposit_program_terms pt
         JOIN deposit_programs p ON p.id = pt.program_id
        WHERE pt.program_id = $1 AND pt.term_months = $2`,
    ...
    if (termRes.rows.length === 0) {
      ...
      data.deposit_type = term.deposit_type;
      data.interest_payment = term.interest_payment;

...
public class KMeans : Algorithm
{
    ...
    public KMeans(IEnumerable<PointF> points, int clustersCount)
    {
        ...
        // Используем Maximin вместо случайной инициализации
        var initializer = new MaximinInitializer();
        var initialCenters = initializer.Initialize(Points, clustersCount);
            
        Clusters = initialCenters
            .Select(c => new ClusterDots(c))
            .ToList();
    }
